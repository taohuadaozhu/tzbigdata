...
<div class="container">
  ...
  <div class="page-content" v-if="pageActive===2">
    <div class="normal-content clearfix">
        <p class="line-height-32 ng-binding">当前选中的表头为第 <span class="a-with-color ng-binding">3</span> 行，如不符合预期，可点击将你需要的数据行指定为表头和设置表头类型.</p>
   ...
   <div class="page-content" v-show="pageActive=== 3">
     <table>
       <thead><tr><td>数据表</td><td>所属文件夹</td><td>分类标签</td><td>备注</td><td>是否公开</td></tr></thead>
       <tbody>
         <tr v-for="item in sheetData">
          ...
          <td><input type="text" class="t-input"></td>
          <td><textarea rows="2" style="width:250px;"></textarea></td>
          <td>
            <el-switch
              v-model="value"
              on-text="公开"
   ...
   <div style="padding:20px;"><el-button type="text" v-show="pageActive!=1" @click="goback()">上一步</el-button><el-button @click="gonext()" style="margin-left:20px;" type="primary" v-show="pageActive!=1">{{pageActive===2?'下一步':'保存'}}</el-button></div>
  </div>
</template>
<script>
